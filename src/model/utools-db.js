...
// 具体的剪切板内容就不进行同步了，没必要，所以只需要在【定制】【设置】页面发生数据改变时写入此db即可  --待开发
// 后续将会配置导入导出功能（这样非会员也能通过导出的配置文件实现多端同步）--思路：届时需在导入时将导入文件的配置项写入该DB/导出时将此db的配置项写入导出文件 --待开发，此功能应该是大致功能完善后才会开发
class UDB {
  //1. 构造函数，传入的是存储的_id
  constructor(key) {
    //初始化存储的_id、存储的对象
